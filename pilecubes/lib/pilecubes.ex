...
defmodule Pilecubes do
  def calc_cubes(n, current_calc, count, stop_at) when current_calc >= stop_at do
    if current_calc === stop_at do
      count - 1
    else
      -1
    end
  ...
  def calc_cubes(n, cc, count, stop_at) do
    m = n - count
    calc_cubes(n + 1, cc + n * n * n + m * m * m, count + 1, stop_at)
  end
